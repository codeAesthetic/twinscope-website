---
title: JSON
description: A structural JSON tree rather than a line diff — reformat a file and nothing changes, reorder an array and it is still the same array, with every suppressed difference named.
lede: A structural tree, not a line diff. Reformat a file and nothing changes; reorder an array and it is still the same array. Everything normalisation hides is counted and one click from coming back.
---
...
<MediaFigure
  id="json-tree"
  alt="A JSON comparison as a tree of rows, with container badges and before-to-after values"
  caption={
    <>
      <b>One row per node.</b> Containers summarise what is below them, so a collapsed tree is still
      readable.
    </>
  }
/>
...
- **Expand all** opens every container; the twisty on any container row collapses just that one.
