...
2. **The same basename elsewhere.** `src/a.ts` → `src/lib/a.ts` is a move even when the content also changed — so a **cross-directory** rename is found, not just one within a folder.
3. **Similar content.** Chunk hashes over a sample, plus size proximity, for a file that was renamed _and_ edited.

<Callout tone="note" title="Two floors, both learned the hard way">
  A file under **64 bytes** is judged on its name alone. At 27 bytes, `export const gone = true;` and
  `export const added = true;` share most of their sampled chunks and score as a rename — so a tiny
  file needs a stronger signal than content similarity.

And scoring is |removals| × |additions|, each comparison possibly reading a file. Past a cap of
**4,000 pairs** it falls back to the older name-and-size rule and **says so in a note**, because a
reader needs to know why the renames look worse than usual.
